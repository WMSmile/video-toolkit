...
func (w *WindowsRecognizer) Process(samples []float32) string {
	if w == nil || w.recognizer == nil || w.vad == nil || len(samples) == 0 {
		return ""
	}

	// 1. 将样本喂入 VAD 内部缓冲区
	w.vad.AcceptWaveform(samples)
	...
	// 如果有标点模型，则转换
	if w.punc != nil && rawText != "" {
		// 这里的 AddPunct 会把 "你好吗我很好" 变成 "你好吗？我很好。"
		return w.punc.AddPunct(rawText)
	}
...
func (w *WindowsRecognizer) Flush() string {
	if w.vad == nil {
		return ""
	}

	// 某些版本的 sherpa 可以在此处通过输入一组静音采样来强制触发检测
	// 或者如果库支持，直接调用 w.vad.Flush()
...
func (w *WindowsRecognizer) Close() {
	if w.vad != nil {
		// 释放 VAD C内存
		sherpa.DeleteVoiceActivityDetector(w.vad)
		w.vad = nil
	}
	if w.recognizer != nil {
