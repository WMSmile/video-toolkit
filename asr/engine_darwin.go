...
func (d *DarwinRecognizer) Process001(samples []float32) string {
	if d.recognizer == nil || d.vad == nil || len(samples) == 0 {
		return ""
	}
	...
	fmt.Printf("VAD 状态1: %v\n", d.vad.IsSpeech())
	fmt.Printf("VAD 状态1: %v\n", d.vad.IsEmpty())
	// 2. 检查 VAD 是否识别出了完整的语音段
	for !d.vad.IsEmpty() {
		fmt.Printf("VAD 状态2: %v\n", d.vad.IsSpeech())
		// 取出最前面的语音段
		segment := d.vad.Front()
		...
		res := stream.GetResult()
		fmt.Printf("Process 识别结果: %s\n", res.Text)
		if res.Text != "" {
			resultTexts = append(resultTexts, res.Text)
		}

		// 弹出已处理的片段（这一步释放内部内存，防止暴涨）
		d.vad.Pop()
	}

	return strings.Join(resultTexts, "")
}

func (d *DarwinRecognizer) Process(samples []float32) string {
	if d == nil || d.recognizer == nil || d.vad == nil || len(samples) == 0 {
		return ""
	}
	...
	// 如果有标点模型，则转换
	if d.punc != nil && rawText != "" {
		// 这里的 AddPunct 会把 "你好吗我很好" 变成 "你好吗？我很好。"
		return d.punc.AddPunct(rawText)
	}
...
func (d *DarwinRecognizer) Flush() string {
	if d.vad == nil {
		return ""
	}

	// 某些版本的 sherpa 可以在此处通过输入一组静音采样来强制触发检测
	// 或者如果库支持，直接调用 d.vad.Flush()
